...
class NaiveVariationalFDivergenceEstimator(FDivergenceEstimator):
    ...
    def _calculate_batch_loss(self, batch: Tuple[Tensor, Tensor, Tensor]) -> Tensor:
        ...
        clipped_log_p_over_q = torch.clip(log_p_over_q_hat, -14, 14)
        return - self.divergence.naive_estimate_given_log(clipped_log_p_over_q, labels, weights)
    ...
    def _accumulate_validation_Df(self, batch: Tuple[Tensor, Tensor, Tensor]) -> None:
        ...
        log_p_over_q_hat = self.model(x)[:, 0]
        clipped_log_p_over_q = torch.clip(log_p_over_q_hat, -14, 14)
        p_summands, q_summands = self.divergence.calculate_naive_rep_summands_given_log_by_label(clipped_log_p_over_q, labels)
